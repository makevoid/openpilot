...
ACCEL_MAX = 0.7 # m/s2 ( 1.5 is default, even if I like it at around 1.8, looking for a wifey-optimized chill rides :) )
ACCEL_MIN = -3.0 # 3   m/s2
ACCEL_SCALE = max(ACCEL_MAX, -ACCEL_MIN)

ACCEL_MAX_LOWSPEED = 1.7 # m/s
...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    # Cut steering for 1.5s after fault
    if not enabled or (frame - self.last_fault_frame < 150):
      apply_steer = 0
      apply_steer_req = 0
    else:
      apply_steer_req = 1
